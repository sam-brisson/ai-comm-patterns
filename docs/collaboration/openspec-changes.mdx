...
import GitHubProjectBoard from '@site/src/components/GitHubProjectBoard';
...
Want to propose a new change or explore an idea? Create an issue with a conversation transcript and label it `propose` or `explore` to kick off the process.

<GitHubProjectBoard owner="sam-brisson" repo="ai-comm-patterns" />
