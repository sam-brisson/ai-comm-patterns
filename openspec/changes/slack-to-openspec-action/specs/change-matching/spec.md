...
### Requirement: Match transcript to existing changes
The system SHALL compare transcript content against all existing OpenSpec changes and identify which change(s) the transcript relates to.

#### Scenario: Clear match to single change
- **WHEN** transcript discusses "spec viewer redesign" and `spec-viewer-v2` change exists
- **THEN** system matches to `spec-viewer-v2` with high confidence

#### Scenario: Multiple potential matches
- **WHEN** transcript touches topics relevant to both `spec-viewer-v2` and `dark-mode-support`
- **THEN** system identifies both changes with confidence scores for each

#### Scenario: No existing change matches
- **WHEN** transcript discusses a new initiative with no matching change
...
- **WHEN** transcript explicitly mentions change name or key proposal terms
- **THEN** system assigns high confidence (>80%)

#### Scenario: Low confidence match
- **WHEN** transcript has tangential relevance to a change
- **THEN** system assigns low confidence (<50%) with explanation

### Requirement: Suggest new change names
The system SHALL propose a kebab-case change name when transcript represents a new initiative.

#### Scenario: New feature discussion
- **WHEN** transcript discusses "adding keyboard shortcuts for power users"
- **THEN** system suggests change name: `keyboard-shortcuts`

#### Scenario: Bug fix discussion
- **WHEN** transcript discusses "fixing the login timeout issue"
...
- **WHEN** system matches transcript to `spec-viewer-v2`
